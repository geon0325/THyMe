...
int main(int argc, char *argv[])
{
	...
	vector<double> motif2count(96, 0);
	vector<int> intersection(E, 0);

	mt19937 gen(SEED);
	uniform_int_distribution<> dist(0, E-1);
	uniform_real_distribution<> dist_T(0, T);
	...
	for (int num_sample = 0; num_sample < S; num_sample++){
		...
		// Sample an interval
		int s = dist(gen);
		double t_s = hyperedge2time[hyperedge2order[s]];

		double T_start = (double)(t_s - (long long)dist_T(gen));
		...
		vector<double> motif2count_samp(96, 0);
		vector<long long> upd_time(E2V.size(), -1LL);
		...
		for (int end = start_index; end < E; end++){
			...
			if ((T_start <= time_a) and (time_a < T_end)){
				...
				for (const hmotif instance: instances){
					...
					motif2count_samp[motif_index_bc] += (double)bc_cnt;
					motif2count_samp[motif_index_cb] += (double)cb_cnt;
				}
				...
				for (int i = 0; i < deg_a; i++){
					...
					motif2count_samp[motif_index_ab] += (double)ab_cnt;
					motif2count_samp[motif_index_ba] += (double)ba_cnt;
					motif2count_samp[motif_index_bb] += (double)bb_cnt;
				}

				// Temporal h-motif 96
				int a_temp_cnt = (int)E2times[index_a].size();
				if (a_temp_cnt >= 2) motif2count_samp[95] += (double)((a_temp_cnt - 1) * (a_temp_cnt - 2) / 2);
			}
		}
		double q = (double)num_hyperedges / (T * E);
		for (int i = 0; i < 96; i++){
			motif2count[i] += motif2count_samp[i] / q;
		}
	}

	vector<double> final_motif2count(96, 0);
	for (int i = 0; i < 96; i++){
		final_motif2count[i] = motif2count[i] * (1.0 / (S * T));
	}
