...
int binary_search(vector<double> index2time, double t, int i, int j)
{
	if (t <= index2time[0]) return 0;
	else if (i < j){
		int mid = i + (j - i) / 2;
		if (index2time[mid] == t)
			return mid;
		else if ((index2time[mid] < t) and (t <= index2time[mid+1]))
			return mid+1;

		if (index2time[mid] < t)
			return binary_search(index2time, t, mid, j);
		else if (index2time[mid] > t)
			return binary_search(index2time, t, i, mid);
	}
	return -1;
}
...
int main(int argc, char *argv[])
{
	...
	sort(hyperedge2order.begin(), hyperedge2order.end(), [&](long long i, long long j){
			if (hyperedge2time[i] == hyperedge2time[j]) i < j;;
	...
	vector<double> index2time;
	for (int i = 0; i < E; i++){
		double e_i = hyperedge2order[i];
		double time_i = hyperedge2time[e_i];
		index2time.push_back(time_i);
	}
	...
	mt19937 gen(SEED);
	uniform_real_distribution<> dist(T_min - T + 1, T_max + 0.999999);
	...
	for (int num_sample = 0; num_sample < S; num_sample++){
		...
		// Sample an interval
		double T_start = (double)(long long)dist(gen);
		double T_end = T_start + T;
		
		int start_index = binary_search(index2time, T_start - delta + 1, 0, E-1);
		...
		for (int end = start_index; end < E; end++){
			...
			if (time_a >= min(T_end, T_max)) break;
	...
	for (int i = 0; i < 96; i++){
		final_motif2count[i] = motif2count[i] * ((T_max - T_min + T) / (S * T));
	}
